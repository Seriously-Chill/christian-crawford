...
Every heading (`display` through `h5`), every button and nav label, and the sitewide default body text (`body`) are set in **Montserrat** — the kit defines two other real Google Fonts, Roboto and Roboto Slab, as its "Primary"/"Secondary"/"Text" global font slots, but neither appears applied anywhere on the captured page, so treat them as loaded-but-unconfirmed rather than in active use. Letter-spacing runs tight and negative (−0.02em to −0.07em, tightening as size increases) — carry that on any new heading rather than defaulting to 0.
